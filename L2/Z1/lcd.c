...
{
	PORTD |= 0x02; 								// Wysy³am flagê ENABLE
	PORTD = (PORTD & 0x0F) | (data & 0xF0);		// Wysy³am pó³ bajta
	PORTD &= 0xFD;								// Usuwam flagê ENABLE
}
...
void LCD_CLEAR()
{
	PORTD &= 0xFE;								// Prze³¹czam LCD w tryb wprowadzania komend
	_delay_ms(2);								// OpóŸnienie w celu wykrycia stanu "0" na pinie RS
	sendCommand(0x01);							// Czyszczê wyœwietlacz
	PORTD |= 0x01;								// Prze³¹czam LCD w tryb wprowadzania danych
}
...
void LCD_INIT()
{
	...
	DDRD |= 0xF3;								// Ustawiam czêœæ linii A
